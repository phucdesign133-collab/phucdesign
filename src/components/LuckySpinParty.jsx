...
import { useNavigate } from 'react-router-dom'; // Import thư viện điều hướng chuẩn
import logo from '../images/logo/v1.png'; // Trỏ đúng đường dẫn ảnh logo của anh
...
export default function LuckySpinParty() {
  const navigate = useNavigate(); // Khởi tạo hook điều hướng trang
  ...
  // Theo dõi kích thước màn hình để tự co giãn đĩa quay Canvas
  useEffect(() => {
  ...
  // Tính toán kích thước đĩa quay tối ưu theo thiết bị (Mobile: ~300px, Desktop: 440px)
  ...
  // Hàm vẽ vòng quay thích ứng size động
  const drawWheel = (currentAngle) => {
    ...
    currentPrizes.forEach((prize, i) => {
      ...
      // Tự thu nhỏ font chữ khi ở trên điện thoại
      ctx.font = windowWidth < 500 ? 'bold 11px "Quicksand", sans-serif' : 'bold 14px "Quicksand", sans-serif';
      ctx.translate(center, center);
      ctx.rotate(angle + arc / 2);
      ctx.textAlign = 'right';
      // Thay đổi khoảng cách text dựa theo bán kính đĩa
      ctx.fillText(`${prize.icon} ${prize.text}`, radius - (windowWidth < 500 ? 15 : 25), 4);
      ctx.restore();
    });
    ...
    ctx.font = windowWidth < 500 ? 'bold 9px sans-serif' : 'bold 13px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('PHÚC', center, center - (windowWidth < 500 ? 1 : 2));
    ctx.fillText(isPremiumMode ? 'VIP' : 'BASIC', center, center + (windowWidth < 500 ? 9 : 14));
  };

  // Vẽ lại mỗi khi đổi chế độ hoặc đổi kích thước màn hình
  useEffect(() => {
  ...
  const spinTheWheel = () => {
    ...
    const animateSpin = () => {
      ...
      const deltaAngle = spinAngleStart - easeOut(spinTime, 0, spinAngleStart, spinTimeTotal);
  ...
  const calculateFinalPrize = () => {
    ...
    const index = Math.floor((360 - (degrees % 360)) / arcd) % currentPrizes.length;
  ...
  const handleSendToZalo = () => {
    ...
    window.open(`https://zalo.me/${ZALO_PHONE_ADMIN}?text=${encodeURIComponent(message)}`, '_blank');
  };

  // Check xem màn hình hiện tại có phải mobile không để điều chỉnh layout bằng CSS Inline
  ...
  return (
    <div style={{
      ...
      {/* NÚT MŨI TÊN QUAY LẠI CHẠY MƯỢT MÀ KHÔNG LỖI */}
      <button onClick={() => navigate('/')} style={styles.backButton}>
  {/* Thẻ img chuẩn React: src phải bọc trong dấu ngoặc nhọn {logo} chứ không để trong dấu nháy kép */}
  <img src={logo} alt="Phúc Design" style={{ height: '35px', objectFit: 'contain' }} />
</button>
  ...
  />
          ...
          {prizeResult && (
            <div style={{
              ...
              <h2 style={{...styles.resultText, fontSize: isMobile ? '18px' : '20px'}}>{prizeResult.icon} {prizeResult.text}</h2>
...
const styles = {
  ...
  pointer: {
    ...
    width: '0', height: '0', borderLeftStyle: 'solid', borderLeftColor: 'transparent',
    borderRightStyle: 'solid', borderRightColor: 'transparent', borderTopStyle: 'solid', zIndex: 10
  },
  canvasStyle: { borderRadius: '50%', background: '#13111c', maxWidth: '100%' },
  spinButton: { marginTop: '20px', color: '#fff', border: 'none', fontWeight: 'bold', borderRadius: '30px', cursor: 'pointer' },
  disabledBtn: { background: '#334155', cursor: 'not-allowed' },
  formWrapper: { background: 'rgba(255, 255, 255, 0.02)', padding: '25px', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.06)', boxSizing: 'border-box' },
  formTitle: { margin: '0 0 18px 0', textAlign: 'center' },
  inputGroup: { marginBottom: '15px' },
  label: { display: 'block', marginBottom: '6px', fontSize: '12px', fontWeight: 'bold' },
  input: { width: '100%', padding: '11px', borderRadius: '10px', border: '1px solid #3f3f46', background: '#09070f', color: '#fff', outline: 'none', boxSizing: 'border-box' },
  ...
  codeBadge: { background: '#ffffff', color: '#111111', padding: '6px 12px', borderRadius: '6px', display: 'inline-block', fontWeight: 'bold', fontSize: '16px', letterSpacing: '0.5px' },
  zaloSubmitBtn: {
    marginTop: '12px', width: '100%', background: '#0068ff', color: '#fff', border: 'none', padding: '11px',
